...
**铁律摘要**：Value 层库级，不碰 bytes/socket；v1 create → unsupported_schema_version / InternalContractViolation fail closed；错误码以 IC §5.7 为唯一来源；三方法（task.list / event.subscribe / event.poll）缺 handler 前禁止 server。完成度开工前查 MATRIX/PROGRESS。
