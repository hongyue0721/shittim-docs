...
3. [`adr/`](adr/README.md)记录未被规范覆盖的局部实施决策；accepted不等于代码全部完成。ADR-0009将里程碑定为`V2InitialBuildActive`并采用fresh baseline；切片0–4c全部落地（11/11 High 闭合：Approval 撤销 Lease 与真实远程验签均已落地），Lease/Stop Fence 全链（ADR-0011，migration 0010）与切片5 child materializer（migration 0011）已落地并通过独立审查，切片6 `stop.activate` / `stop.status` handler 已落地；`task.list` / `event.subscribe` / `event.poll` 仍无 handler，server 阶段门保持关闭。ADR-0010将Policy matcher固定为直接消费active v2合同，并直接退役三项未投产旧Policy合同；当前Schema基线为`80 = 38 retained + 42 component-native`。
