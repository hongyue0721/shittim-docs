...
- `NormalizedChildTaskProposalV1`在本阶段由`kernel-task-creation`跨Rust/未来TS得到相同JCS与hash；`ChildTaskDeltaProjectionV1`、`MaterialAuthorizationProjectionV1`、`ObservationEvidenceProjectionV1`及`SubjectProjectionV1`仍属于未来专门authorization/subject projection切片，不得把其实现或fixture计入task creation crate。本阶段仍需保证URI/数组/时间纳入字段改变时child proposal hash按合同变化；
- `kernel-task-creation`必须作为本阶段root/child proposal normalization、root receipt/idempotency、child proposal/receipt hash与root/child allocation validation的唯一生产owner；依赖`kernel-contracts`并调用`domain-policy`唯一URI parser，不依赖SQLite/KCP、不分配ID、不读repository、不写存储，全部事实由caller typed input注入。`ChildTaskDeltaProjectionV1`、`MaterialAuthorizationProjectionV1`、`ObservationEvidenceProjectionV1`不在本crate本阶段；`domain-task`不得增加policy依赖。长期中立URI crate必须另立ADR，当前不得复制第二实现；pure library已实现并有inline unit/doctest；official fixtures与harness已完成（production owner harness + 独立CLI进程/Schema路径 + stored bytes/hash自一致性共享唯一JCS authority），repository/handler/materializer与cutover仍未完成；
...
- Event v2 Schema切片必须精确新增IC §13.6.2的八项，且当前仍为contract-only。逐项断言exact title/ID/source/component/kind/version/compatibility/schema_version_field/whole-schema direct refs；`ConfirmationModeV1`必须归common，event `allowed_refs`扩为`common,policy`后DAG保持`event→policy→common`无环；禁止第九个平行enum/近义Schema。实现后manifest精确61项（41 retained + 20 component-native），production bindings仍空；三enum闭集精确为confirmation `generic|local|system_authentication|remote_signature|plan_revision`、record `request|resolution|invalidation`、subject `operation|task_proposal|plan_revision`；
...
- `ActionStateChangedPayloadV1`逐字段required/required-nullable、整数/时间/UUID/unique array与Schema条件有正反例。`ApprovalStateChangedPayloadV1.change_kind`四值enum必须与四branch双射，逐格覆盖from/to kind及`from_head_ref,to_head_ref,request_ref,resolution_ref,invalidation_ref,replacement_request_ref`的required null/non-null真值表；特别证明initial与replacement同`to_record_kind=request`仍不歧义。另测repository exact equality：initial `to=request`、resolution `from=request/to=resolution`、invalidation/replacement旧head/ref关系、replacement predecessor；同时明确Schema不能表达任意字符串等值及repository必须验证合法状态边、revision+1、generation/PD/Approval/canonical chain关系；
- `EventEnvelopeV2`字段精确沿用v1集合并固定schema_version=2；不得新增`emitted_at`或把`delivered_at`放入Envelope；outbox_position为正ASCII十进制，本批兼容v1继续接受前导零，repository重建输出无前导零普通十进制。五类type enum与五个conditional mapping双射，复用三个retained payload v1并绑定两个新payload v1，证明Envelope/payload版本正交；
- active Event Catalog候选谓词必须逐项负测：reserved exact ID/title/source任一占用；结构候选要求`component=event,kind=envelope,schema_version.const=2,root type closed enum,allOf type-const→aggregate const+whole-schema payload ref`；普通event object/payload/envelope不得误抓。候选0/重复/partial/mixed metadata全部fail closed。exact root的五值enum/五mapping双射覆盖whole-schema ref、inline、fragment、missing、duplicate、enum无branch、branch无enum、单branch多type和额外branch；生成精确包含named `EventTypeBinding { event_type:&'static str, aggregate_type:&'static str, payload_schema_id:&'static str, payload_schema_version:u64 }`、private fixed-size binding/type arrays及public `EVENT_ACTIVE_BINDINGS`、`EVENT_LEGACY_V1_BINDINGS`、`EVENT_ACTIVE_TYPES`、`EVENT_LEGACY_V1_TYPES` slices。bindings顺序必须分别等于v2/v1 Envelope root type enum声明顺序，type arrays由通用`const fn project_event_types<const N: usize>`从bindings投影，event type字符串只在binding初始化出现一次；生成artifact真实cargo编译通过，不得保留`EVENT_V1_TYPES`alias或第二份平行mapping/type数据；typed decode按Envelope版本选择且未知type/version/payload mismatch fail closed；
- migration 0003 mixed Outbox测试从真实v1数据库升级：descriptor identity精确固定`migration_version=3`、`name=versioned_event_outbox`、唯一SQL asset path=`rust/crates/kernel-sqlite/migrations/0003_versioned_event_outbox.sql`；该单asset覆盖ledger ALTER、replacement table、全部index/constraints/table-swap DDL，Rust transform在同一transaction执行。先在事务内把现有四列ledger升级为含`descriptor_hash/descriptor_format_version`，历史0001/0002保持SQL-only checksum+null descriptor；新0003 format-v1模板只允许`algorithm_id:<stable transform algorithm id>`语义，algorithm必须是具体transform稳定ID而非族名。0003 exact三元组固定`shittim.kernel-sqlite.outbox-v1-to-versioned-v1`/`1`/`kernel_sqlite::migration::outbox_v1_to_versioned_v1`，与唯一asset raw SHA组成完整JCS descriptor，single hash同时写checksum/hash；同version任一asset/name/algorithm/version/implementation漂移、未知format/三元组、半填descriptor均`migration_drift`。同一outbox保留position/sequence/delivery/dedup/payload，`causation_kind/id`变为JCS `causation_json`，不建`outbox_v2`；DB CHECK覆盖schema_version×event_type×五类aggregate映射，JSON列只承诺valid root object；逐row验证JCS、v1/v2 exact decode、row count/hash/ref、MAX/next position、aggregate 0..N与sequence table闭包。任一legacy corrupt row、descriptor/transform/count/readback失败使ledger ALTER+DDL+copy+row整体rollback；旧binary对成功升级库返回`database_schema_too_new`，版本回滚只允许恢复备份；
- v1/v2 append共用一个私有`append_versioned_event` SAVEPOINT/sequence/position内核；public类型由`kernel-sqlite/src/outbox.rs`拥有并从crate root导出，精确为`StoredEventEnvelope::{LegacyV1(TypedEventEnvelope),ActiveV2(TypedEventEnvelopeV2)}`、`OutboxRecord { envelope, delivered_at }`、逐字段保持现有caller事实的`PendingLegacyEventV1`、closed `EventAggregateId::{Task(Uuid),Action(Uuid),ApprovalChain(Uuid),StopFenceGlobal}`及`PendingActiveEventV2 { event_id:Uuid,aggregate_id:EventAggregateId,occurred_at,causation_ref:CausationRefV2,correlation_id,dedup_key,payload:EventEnvelopeV2Payload }`。active pending不得有caller可写event_type/aggregate_type/schema_version/sequence/position；store从payload variant+generated binding唯一派生type/aggregate并在分配前核对aggregate ID enum/payload ID，correlation/dedup非空。public写方法精确为`append_legacy_event_v1`/`append_active_event_v2`，旧`PendingEvent/append_event`无alias。两版本append失败即使外层继续commit也不占号；mixed read按position稳定排序；corrupt row不推进cursor且不能mark delivered；writer contention、unhealthy/rollback与existing mark-delivered语义保持；cursor与v1/v2 Envelope本批继续接受前导零输入，DB重建输出普通十进制，不在0003暗中收紧；
- retained `event.poll` response v1的events item exact为EventEnvelope v1，绝不允许返回v2。mixed Outbox只证明store/Publisher可读，不等于KCP poll mixed可用；在未来versioned KCP response/MethodVersionBinding/handler切片完成前，v1 poll遇到下一条v2不得跳过、降解或推进cursor，且不得启用可连接handler；
- 本次八Schema只实现`ActionTransitionRefV1` wire/ref；`ActionTransitionIntentV1`必须在后续task component `domain_object` Schema批次（exact ID/source见IC §6.14）与Action持久对象/repository/migration同批实现。compile/API负测禁止producer在此之前引入临时intent struct/自由JSON或宣称producer闭环；
...
- 当前文档切片本身的机械检查必须证明schemas/manifest与generated artifacts未变、production MethodVersionBindings仍为空、现状仍为53 Schema，并通过完整`check-schema.sh`与`git diff --check`；不得把contract-only写成Schema或实现完成；
...
- `validate_production_manifest_stage`独立断言当前`schemas/manifest.json` bindings仍为空，并由production CLI `check`/`generate`在load后、plan/render前调用；非空synthetic binding catalog的end-to-end plan/render测试必须走library API并使用显式non-production registry profile，不调用production CLI stage gate。synthetic registry不靠路径、env、fixture目录名或调用栈隐式特判，production CLI始终调用该stage gate；这证明empty是仓库阶段事实而非loader通则；generated binding catalog只证明library facts，不证明dispatcher/handler/server/SDK可用；V2ProductionWriteCutover前不得启用production binding；
