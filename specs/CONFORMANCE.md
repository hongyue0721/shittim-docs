...
- Policy confirmation词表全映射：`domain-policy::evaluate_policy`直接接收`PolicyRuleV2`，draft直接使用`PermissionDecisionV2Decision`；五种mode逐一映射PD decision与Approval request canonical wire；`remote_signature→require_remote_signature`是正常Policy结果，参与priority、specificity、winner-only rate-limit与耗尽fallback；不得存在v2→v1转换、Generic probe、remote rule-ID side table或mode/decision adapter。未投产旧PolicyRule/PermissionDecision/ApprovalRecord合同按ADR-0010从source/manifest/ledger/generated/validation入口直接退役；`V1`后缀本身不等于legacy；
...
- 完整 family Envelope Schema、方法 payload Schema 与 generated typed decode 对八方法逐项有 Accepted 用例；三个已知未实现方法也必须先完整 decode，畸形 payload 不得提前变成 KnownCatalogMethodNotImplemented；
...
- 三个已知但未注册方法 narrow 后得到不可序列化的本地 `KnownCatalogMethodNotImplemented`，不能成为 `KcpError`、`unsupported_method`、`method_unavailable` 或 `internal_error`；五个 registered variant 分别只路由 ping/create/get/stop.activate/stop.status；
- dispatcher 构造只复用现有 `KernelClock`、`KernelIdGenerator`、`TaskApplicationBackend`、`StopApplicationBackend`，按 ping/create/get/stop.activate/stop.status variant 传递所需端口，不创造平行接口；它不重复 Schema/deadline、不改写 `HandlerResult`；fake port 矩阵证明 response、ContractFailure 与 post-commit notification intents 均无损透传，family/discriminator/payload variant 错配不能进入 RegisteredRequest；
...
- 未实现三方法 handler、bytes/frame/transport/server 的情况下组合根必须拒绝启动 server；不得以“运行时返回 known-unimplemented”替代启动阶段完整性检查；
- typed application handler 输入必须已经通过对应 Envelope Schema、方法 payload Schema 与 typed decode；正常 dispatcher 路径还必须先完成五方法 registration narrow。现有公共 `handle_*` 的错配输入继续产生本地 InputMethodMismatch，但 handler 不得重复 protocol/auth/method/schema 分类，也不得把内部错误重新归为 `invalid_request`；
- `system.ping`、`task.create`、`task.get`、`stop.activate`、`stop.status` handler 使用 fake backend/fake clock/deterministic fake ID generator 做库级矩阵；Value preflight/registration/dispatcher 同样只做不可连接库级测试，本阶段不得启动 Socket/Named Pipe 或构造可连接 server；
- 五方法第一次可观察操作是 clock 读取，并在任何 ID/backend 前按 `now >= deadline` 检查；入口已过期不访问 backend、不分配 ID；
...
- V2InitialBuildActive切片1a精确新增IC §13.6.3四项；历史快照：该批落地时manifest=65（当时41 retained + 24 component-native），切片1b后为70，production bindings当时仍空。自动测试逐项断言exact title/ID/source/component/kind/version/compatibility/schema_version_field/direct refs，component DAG为`audit/task→common,policy→common`且无环；该阶段retained entry、ledger与source bytes不变；
- V2InitialBuildActive切片1b精确新增IC §13.6.4五项；历史快照：该批落地时manifest=70（当时41 retained + 29 component-native），切片1c-i后为75、1c-ii后为83，production bindings当时仍空。自动测试逐项断言exact identity/direct refs、typed round-trip、required/unknown/bounds、联合分支/nullability、projection set/multiset/URI/time/hash稳定性、pseudo-provider拒绝、retained VerificationResult复用边界及四份authorization projection official fixtures/harness/oracle；
- V2InitialBuildActive切片1c-i精确新增IC §13.6.5五项；历史快照：该批落地时manifest=75（当时41 retained + 34 component-native），production bindings当时仍空。自动测试覆盖五root identity/DAG、PermissionDecision mode映射、PolicyRule闭集、Approval双层tagged union、三Subject branch、request challenge/nullability、resolution专属证据槽、旧字段泄漏、Allocation UTC-second/CausationRefV2，以及SubjectProjection三branch official fixture/JCS/hash/逐字段tamper；
- V2InitialBuildActive切片1c-ii精确新增IC §13.6.6八项；历史快照：该批完成时manifest=83（当时41 retained + 42 component-native），production bindings当时仍空；ADR-0010退役三项旧Policy合同后当前为80（38+42）。自动测试覆盖八root identity/DAG、RemoteSignatureAlgorithm tagged union、allowed_decisions精确有序array const、nonce/key/signature长度、challenge state闭集、local/system evidence闭集、response禁字段、preimage JCS/SHA-256、旧字段泄漏及八份official fixtures；
...
- 当前Schema机械检查必须证明manifest=`80 = 38 retained + 42 component-native`、production MethodVersionBindings精确为IC §13.5八方法集、source→generated byte稳定、失败无partial artifacts；旧PolicyRule/PermissionDecision/ApprovalRecord三项schema ID、source path、generated type/catalog/validator、example和contract_validation入口必须全无，active v2三项必须保留。Event migration/Outbox focused tests覆盖descriptor raw bytes/hash、ledger shape/too-new、空表0003升级与非空 legacy reinitialize-required、v2-only事件矩阵、共享position/sequence、active aggregate mismatch、stored corruption（含 schema_version=1）/delivery gate、savepoint poison、migration 0005 drop死表；并通过full `check-schema.sh`与`git diff --check`。不得把Publisher、versioned KCP poll、`task.list`/`event.subscribe`/`event.poll` handler或server写成已完成（child materializer 已随切片5 落地，可如实宣称）；
...
- integer range专项：neutral IR记录inclusive min/max而不含Rust名；V2 Envelope payload `schema_version`编译时类型断言精确为`u32`，`u32::MAX` Schema/serde round-trip通过，`u32::MAX + 1` Schema拒绝；没有完整安全范围的当前38 retained integer public bytes保持既有`i64`；
