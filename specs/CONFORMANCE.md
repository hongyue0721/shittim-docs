...
- 当前Event v2前两实现段的机械检查必须证明manifest=61、production MethodVersionBindings仍为空、source→generated byte稳定、失败无partial artifacts；migration/Outbox focused tests覆盖descriptor raw bytes/hash、ledger shape/too-new、真实v1升级与corruption rollback、mixed 3 legacy + 5 active类型矩阵、共享position/sequence、active aggregate mismatch、stored corruption/delivery gate及savepoint poison；并通过full `check-schema.sh`与`git diff --check`。不得把active business producer、Publisher、KCP poll mixed cutover或runtime写成已完成；
