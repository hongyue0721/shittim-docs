...
- `kernel-task-creation`必须作为本阶段root/child proposal normalization、root receipt/idempotency、child proposal/receipt hash与root/child allocation validation的唯一生产owner；依赖`kernel-contracts`并调用`domain-policy`唯一URI parser，不依赖SQLite/KCP、不分配ID、不读repository、不写存储，全部事实由caller typed input注入。`ChildTaskDeltaProjectionV1`、`MaterialAuthorizationProjectionV1`、`ObservationEvidenceProjectionV1`不在本crate本阶段；`domain-task`不得增加policy依赖。长期中立URI crate必须另立ADR，当前不得复制第二实现；pure library已实现并有inline unit/doctest；official fixtures与harness已完成（production owner harness + 独立CLI进程/Schema路径 + stored bytes/hash自一致性共享唯一JCS authority），repository/handler/materializer与V2InitialBuildActive仍未完成；
...
- V2InitialBuildActive切片1a精确新增IC §13.6.3四项；manifest=65（41 retained + 24 component-native），production bindings仍空。自动测试逐项断言exact title/ID/source/component/kind/version/compatibility/schema_version_field/direct refs，component DAG为`audit/task→common,policy→common`且无环；41 retained entry、ledger与source bytes不变；
...
- 注：下列 migration 0003/mixed Outbox 条目描述当前已实现代码事实；其 v1 数据升级生产目的已被 ADR-0009 supersede，V2InitialBuildActive 切片 6 将改为旧库 reinitialize-required 并删除 legacy append 入口：
...
- 当前Schema机械检查必须证明manifest=65、production MethodVersionBindings仍为空、source→generated byte稳定、失败无partial artifacts；Event migration/Outbox focused tests继续覆盖descriptor raw bytes/hash、ledger shape/too-new、真实v1升级与corruption rollback、mixed 3 legacy + 5 active类型矩阵、共享position/sequence、active aggregate mismatch、stored corruption/delivery gate及savepoint poison；并通过full `check-schema.sh`与`git diff --check`。不得把active business producer、Publisher、versioned KCP poll或runtime写成已完成；
...
- `validate_production_manifest_stage`独立断言当前阶段production `schemas/manifest.json` bindings状态（`V2InitialBuildActive`完成前为空，完成后精确等于IC §13.5八方法目标表），并由production CLI `check`/`generate`在load后、plan/render前调用；非空synthetic binding catalog的end-to-end plan/render测试必须走library API并使用显式non-production registry profile，不调用production CLI stage gate。synthetic registry不靠路径、env、fixture目录名或调用栈隐式特判，production CLI始终调用该stage gate；generated binding catalog只证明library facts，不证明dispatcher/handler/server/SDK可用；未满足IC §13.7不得宣称production binding/runtime可用；
