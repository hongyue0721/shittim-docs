...
- 当前Event v2实现段的机械检查必须证明manifest=61、production MethodVersionBindings仍为空、source→generated byte稳定、失败无partial artifacts，并通过focused tests、临时精确树full `check-schema.sh`与`git diff --check`；不得把migration 0003、mixed API、producer/Publisher/runtime写成已完成；
