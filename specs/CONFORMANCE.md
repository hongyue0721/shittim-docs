...
- 当前Schema机械检查必须证明manifest=83、production MethodVersionBindings精确为IC §13.5八方法集、source→generated byte稳定、失败无partial artifacts；Event migration/Outbox focused tests继续覆盖descriptor raw bytes/hash、ledger shape/too-new、真实v1升级与corruption rollback、mixed 3 legacy + 5 active类型矩阵、共享position/sequence、active aggregate mismatch、stored corruption/delivery gate及savepoint poison；并通过full `check-schema.sh`与`git diff --check`。不得把method-aware KCP runtime（3b）、Publisher、versioned KCP poll或server写成已完成；
