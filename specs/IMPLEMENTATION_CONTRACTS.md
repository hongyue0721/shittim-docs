...
- 只返回 `outbox_position > after_outbox_position` 的事件并按位置升序排列；空结果合法。**当前 production store 为 v2-only**（ADR-0009 / 切片3c）：`append_legacy_event_v1` / `StoredEventEnvelope::LegacyV1` 已删除，不存在 mixed production Outbox。retained `event.poll` v1 的 response items exact 引用 `EventEnvelope v1`，因此 **poll v1 不得返回 v2**；在独立的 poll/response 升级切片完成前，不得注册/启动会把 v2 行当作 v1 返回、跳过、降解或推进 cursor 的 handler。未来若要返回 v2，必须新增或升级 versioned response payload、MethodVersionBinding、typed handler 与 Conformance；该升级不在 `V2InitialBuildActive` 范围；
...
`CausationRef v2`的`action_transition` branch必须直接`$ref`该对象；branch不得复制字段或另起近义名。`ActionTransitionIntentV1`是先于Action状态事件持久化的不可变事实，全部字段required：`schema_version=1,transition_id,action_id,expected_action_revision,expected_execution_generation,resulting_execution_generation,from_status,to_status,reason_code,correlation_id,created_at`。revision/generation为非负整数，状态必须是领域合法边。`expected_execution_generation`是CAS前Action当前generation；`resulting_execution_generation`是CAS后Action/event payload generation。普通状态边必须两者相等；`approved→leased`必须严格`resulting = expected + 1`，禁止用状态特例在mechanical commit里绕过该投影事实。
...
唯一键固定为`transition_id`全局唯一，以及`(action_id, expected_action_revision, expected_execution_generation, resulting_execution_generation, from_status, to_status, reason_code)`唯一；同一个intent重放必须canonical readback返回原事实，不分配新transition。Durable/recovery surface为`insert_intent`、`get_intent`、`get_for_action_revision(ActionTransitionBusinessKey)`、`reconcile_intent`；没有update/delete。生产状态边只能由Policy、Approval、未来Lease/child/Recovery等命名业务owner验证typed authority fact后，调用crate-internal mechanical commit。mechanical commit只可在同一事务CAS Action从expected revision/expected generation/`from_status`到`to_status`与resulting generation并append唯一`action.state_changed`，event causation精确等于该intent的`ActionTransitionRefV1`；不得暴露可让caller自由提供PD/Approval/Lease/Verification evidence的通用production入口。读取必须校验intent、Action revision/generation、event payload/causation/correlation和唯一关系；缺失或冲突为`stored_data_invalid`。reconcile只返回`prepared|committed|corrupt`，不得补造event或另换transition id。
...
Action transition的权威wire、intent、CAS、唯一键与reconciliation见§6.14；Audit只引用其结果，不重复定义。AuditRecord v1仅保留Schema/fixture历史验证资产；`task.create v1`实现已在切片3c按ADR-0009删除。active producers使用下列完整v2对象；全部顶层字段required，无关联事实显式null/空数组：
...
2. **V2InitialBuildActive 初始交付**：§13.7全部谓词满足时，production manifest非空八方法binding与v2 dispatcher registration、v2 repository write gate一并作为fresh baseline初始交付；不是从旧部署cutover。v1 runtime写路径删除而非双写切换。**切片3a状态**：production bindings与generated `METHOD_VERSION_BINDINGS`/`select_request_version`已激活。**切片3b状态**：kernel-kcp method-aware preflight/dispatcher/handler 已消费 bindings；active create v2 为唯一 kcp production 写入口。**切片3c状态**：legacy sqlite create/Audit/Outbox v1 write 已删；Outbox v2-only；旧库 open reinitialize-required；migration 0005 drop 死 v1 表。
...
3. **明确不在本切片**：KCP preflight/handler/bindings（切片3）；v1 写路径删除与旧库 reinitialize-required（切片3c）；child materializer（切片5）。
...
4. **ports/adapter**：`TaskCreateOperation` 七 UUID；`UuidPurpose::CreationProvenance`；`BackendError` 对齐 active 闭集（无 parent-task 错误；store `ParentTaskNotFound` 已在 3c 删除）；adapter 映射 `RootTaskCreateV2Command`/`create_root_task_v2`。
5. **删除**：kcp 侧 v1 create handler/adapter/ports/dispatcher narrow。`kernel-sqlite` legacy `create_task` 写路径本片保留给切片3c统一删除（及其全部调用方/测试）。

**明确不在本切片**：五方法 handler、server、Publisher/poll、legacy sqlite write 删除、§13.7 闭合。

### 13.6.10 V2InitialBuildActive切片3c：删除 v1 runtime 写路径 + Outbox v2-only + 旧库拒绝（已落地）

1. **删除**：`WriteTransaction::create_task` / `TaskCreateCommand` / `prepare_legacy_v1_create` 及 v1 Audit/Event producer 助手；`append_legacy_event_v1` / `PendingLegacyEventV1` / `StoredEventEnvelope::LegacyV1`；AuditRecord v1 `append_audit`/`get_audit` production API。
2. **保留**：Task/TaskScope/ContentOrigin(v2) 共享严格读路径（`get_task` 等继续服务 root v2；0004 起 Task 可引用 v2 origin）。
3. **Outbox v2-only**：public 仅 `append_active_event_v2`；stored decoder 对 `schema_version!=2` → `stored_data_invalid`；fresh baseline 不可能有 v1 row。
4. **旧库拒绝**：`SqliteStore::open` 在 migrations 后检查 v1 业务标记（v1 content_origins/audit_records/task_create_idempotency 行、outbox `schema_version=1`）→ `StoreErrorCode::StoredDataInvalid`，message 前缀 `reinitialize-required:`；禁止自动清库/隐式升级。
5. **migration 0003**：asset/descriptor identity 不动；transform 对非空 legacy Outbox 直接 reinitialize-required（不执行 v1 数据迁移）；空表仅 shape 升级。
6. **migration 0005**：fresh baseline 在空表前提下 drop `content_origins`(+parent_refs)/`audit_records`/`task_create_idempotency`；非空拒绝。

**明确不在本切片**：child materializer、Action/PD/Approval repositories、其余 active producers、Publisher/poll、§13.7 全谓词闭合。
