...
> **实现状态**：本节的八个Event v2 source Schema、manifest/compiler、generated catalog/typed types与conformance已经实现；SQLite migration 0003、mixed Outbox API、active producer、Publisher与runtime仍未实现。版本化Schema与统一Outbox实施决策见ADR-0008。
...
> **实现状态**：下述CausationRef v2、ActionTransitionRef v1与EventEnvelope v2 source Schema及generated Rust已经实现；SQLite migration 0003、mixed Outbox API与active producer/runtime仍未实现。exact清单与component DAG见§13.6.2及ADR-0008。
...
本批已一次性加入恰好八项，production manifest现为61项（41 retained + 20 component-native）。production `method_version_bindings`继续为空。Schema source、manifest identity、single mapping IR、Event Catalog与typed decode属于已实现事实；migration 0003、mixed Outbox API、producer/Publisher/runtime仍未实现。
