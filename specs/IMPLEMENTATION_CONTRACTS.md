...
EventEnvelope字段与Outbox语义以`CORE_ARCHITECTURE.md`为准。v1 Envelope只用于历史读取；active producers使用EventEnvelope v2与CausationRef v2（`command_request | event | action | action_transition`）。现有payload可维持各自业务payload版本；首批新增`ActionStateChangedPayload v1`与`ApprovalStateChangedPayload v1`。
...
- payload `ActionStateChangedPayload v1`全部required：`{schema_version:1,action_id,task_id,from_status,to_status,action_revision,execution_generation,permission_decision_ref,approval_resolution_ref,materialized_child_task_ref,verification_result_refs,reason_code,changed_at}`；其中三个ref字段required-nullable，verification数组required可空；child-create进入completed时child ref与至少一个Verification ref必须非null/非空；
...
- payload `ApprovalStateChangedPayload v1`全部required：`{schema_version:1,approval_chain_id,from_head_ref,to_head_ref,from_record_kind,to_record_kind,subject_kind,confirmation_mode,request_ref,resolution_ref,invalidation_ref,replacement_request_ref,permission_decision_ref,action_id,reason_code,changed_at}`；`from_head_ref/from_record_kind`在初始request固定null；其余ref按转换支required-nullable，未适用写null；
- 固定转换：初始request `null→request`；resolve `request→resolution`；invalidate without replacement `request|resolution→invalidation`；invalidate with replacement在同一事务形成一个逻辑head转换`request|resolution→request`，payload同时携带`invalidation_ref`与`replacement_request_ref=to_head_ref`，不得发出可被消费者误认作稳定head的中间invalidation事件；
...
`ActionTransitionRefV1`与`ActionTransitionIntentV1`的唯一权威定义在本节，不能由Audit合同或Event页面另起wire定义。
...
同一聚合中，事务内暂时分配但最终回滚的`sequence`不占号；cursor/delivered语义不变。EventEnvelope v1保留历史读取。
