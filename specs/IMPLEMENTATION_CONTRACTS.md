...
> **实现状态**：本节的八个Event v2 source Schema、manifest/compiler、generated catalog/typed types与conformance已经实现；SQLite migration 0003–0006、**v2-only** Outbox API、严格stored decoder、savepoint poison与旧库 reinitialize-required 已实现。root `task.created` 与 `action.state_changed`（切片4a）active producer 已接入；child/approval producer、Publisher与poll仍未实现。版本化Schema与统一Outbox实施决策见ADR-0008（§7/§8 部分被 ADR-0009 supersede）。
...
> **实现状态**：下述CausationRef v2、ActionTransitionRef v1与EventEnvelope v2 source Schema/generated Rust，以及SQLite migration 0003–0006 / v2-only Outbox API已实现；root task.created 与 action.state_changed（切片4a，causation=`action_transition`）producer 已接入；child/approval producer 与 Publisher/poll 仍未实现。exact清单与component DAG见§13.6.2及ADR-0008。
