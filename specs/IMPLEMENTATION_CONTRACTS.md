...
v1包含`parent_task_id`，现有Rust typed handler/SQLite repository实现了root与direct-child创建。自ADR-0006起：

- source Schema/生成类型可保留用于读取、fixture、迁移与历史完整性验证；
- 不得进入active KCP dispatcher、server registration或production write；
- 历史`parent_task_id!=null`记录合法，迁移写`legacy_direct_create_v1` provenance，不补造Action/Approval/PermissionDecision/Verification；
...
> **合同状态**：本节定义active v2权威合同；当前仓库尚未新增本节要求的八个Schema、generated catalog/typed types、SQLite migration 0003或active producer。版本化Schema与统一Outbox实施决策见ADR-0008。

EventEnvelope字段与Outbox语义以`CORE_ARCHITECTURE.md`为准。v1 Envelope只用于历史读取；active producers使用EventEnvelope v2与CausationRef v2（`command_request | event | action | action_transition`）。现有payload可维持各自业务payload版本；首批新增`ActionStateChangedPayload v1`与`ApprovalStateChangedPayload v1`。Envelope版本与payload版本正交：EventEnvelope v2正式复用retained `TaskCreatedPayload v1`、`TaskStateChangedPayload v1`与`StopFenceActivatedPayload v1`。
...
本节记录**现有未发布Rust代码事实**：三个handler仍绑定v1 generated envelope/payload，其中`task.create v1`允许`parent_task_id`。自ADR-0006起，它不是active production合同；后续实现必须以active v2 handler替换dispatcher中的v1 create registration，v1 handler只可留给migration/test工具且不得由server路由。
...
前述结构检查完成后，必须选择该方法+active version对应的generated Envelope/payload Schema验证，再调用对应generated typed decode。普通caller输入不满足完整合同返回`invalid_request`；成功时只返回active-version `Accepted(TypedCatalogRequest)`。Legacy validators必须是独立migration API，不能借active preflight进入dispatcher。
...
PermissionDecision v1保留legacy read。active evaluation写v2不可变记录：
...
引用 SECURITY_PRIVILEGE.md 为权限判定权威，此处定义 production PolicyRule v2存储字段；v1仅按自身lifecycle读取/迁移：
...
ApprovalRecord v1仅legacy read/validation/migration，production write禁止。v2必须由SchemaGraph表达为两层真正tagged union，禁止任何全部optional的平面替代：外层`record_kind`，内层`subject.subject_kind`。公共顶层字段表：
...
| Approval | §6.10.1三种append复合方法、`get_record`、`get_current_head`、`list_history`、`validate_usable_resolution`、`read_legacy_v1`；不可update/delete |
...
| RootTaskCreate | `create_root_v2`、`get_idempotency_record`、`reconcile_root_create`；legacy v1 write不在production接口 |
...
migration lifecycle：preflight统计v1对象/引用/损坏；backup；在隔离transaction写v2 provenance/index/current-head projection；逐条canonical verify和计数/引用闭包；切换active registration前运行legacy-write negative gate；失败rollback。v1 Approval/PD/TaskCreate只保留明确legacy reader/validator；不得从v1 optional target猜造v2 subject，无法无损投影时标记legacy-only/quarantine。
...
> **合同状态**：下述v2对象尚无source Schema/generated Rust/SQLite实现；下一Schema切片的exact清单与component DAG见§13.6.2及ADR-0008。
...
- v1只允许`command_request | event`并永久用于历史读取；
...
同一聚合中，事务内暂时分配但最终回滚的`sequence`不占号；cursor/delivered语义不变。`outbox_position`为正ASCII十进制字符串；本批为兼容v1继续接受前导零，repository从INTEGER position重建时输出无前导零普通十进制，未来收紧必须版本化。`occurred_at`是唯一Envelope业务时间；不新增`emitted_at`，`delivered_at`只属于Outbox metadata。`correlation_id`与`dedup_key`为non-empty opaque string。EventEnvelope v1保留历史读取。EventEnvelope v2必须以五个conditional mapping闭合type→aggregate→payload；Schema可固定type/aggregate、stop fence `aggregate_id=global`与payload shape，但aggregate/payload ID相等、revision/head/transition等跨对象关系由repository验证。

### 6.16 AuditRecord v2 exact wire

Action transition的权威wire、intent、CAS、唯一键与reconciliation见§6.14；Audit只引用其结果，不重复定义。AuditRecord v1保留历史读取与现有`task.create v1`实现。active producers使用下列完整v2对象；全部顶层字段required，无关联事实显式null/空数组：
...
|---|---|
| `root_command_v2` | `command_request_id,entry_point,actor,receipt_ref`; `parent_task_id=null,action_id=null,legacy_source_ref=null` |
| `child_action_v2` | `parent_task_id,action_id,permission_decision_ref,approval_resolution_ref|null,verification_result_ref,proposal_hash,child_task_delta_hash`; command/legacy refs null |
| `legacy_direct_create_v1` | `command_request_id|null,parent_task_id|null,legacy_source_ref`; Action/PD/Approval/Verification全部null；unknown历史保持null |
| `imported_legacy` | `import_batch_ref,external_stable_ref|null,imported_at`; command/action权威refs null |
...
| legacy migration | 只新增Provenance v1/Audit migration事实（若迁移合同要求） | migration time单独记录，不改历史created_at | legacy/import source；不伪造command/action | 默认不发business creation event；migration event需另立Catalog |
...
`PayloadManifestV1`历史wire仍含旧child字段名，只能由legacy reader/migration显式处理；active v2只生成`child_task_id`，禁止同时接受两个字段或自动fallback。Manifest不保存Payload正文，也不承担审批或内容审查。
...
### 13.6.2 下一批 Event v2 八Schema合同（contract-only）

本批尚未落地；实现时必须一次性加入恰好八项，并把production manifest从当前53项（41 retained + 12 component-native）推进到61项（41 retained + 20 component-native）。production `method_version_bindings`继续为空。
...
active claimant、Catalog/typed生成及常量命名以§5.6为准；八项正好闭合五类active Event，不得增加第九个近义enum/schema，也不得把现有retained payload复制成component-native新版本。本Event v2/Outbox文档合同至此无开放设计问题；后续未完成项均必须按本合同实现，不能在实现期重新发明字段、命名、migration identity或KCP poll兼容策略。

### 13.7 V2ProductionWriteCutover唯一谓词

`V2ProductionWriteCutover`为单一原子发布谓词，且仅当以下全部为真：

1. v2 source、manifest entries、generated Rust/TS目标（TS若该发行物声明）与MethodVersionBinding完整且drift clean；
2. Action/PD/Approval/Identity/transition/root/child repositories及handlers均实现canonical readback、CAS、Outbox和错误映射；
3. migration preflight统计/损坏报告完成，备份可恢复，隔离migration成功，逐对象计数/hash/ref/unique closure verify通过；
4. legacy negative gates证明production dispatcher、server registration、repository public write与SDK active client均不能写TaskCreate v1、Approval v1、PD v1；
5. 同一发布事务/部署步骤原子切换active method binding、dispatcher registration与repository write gate。

cutover前：未发布v1内部handler/repository不得启动可连接server。cutover时：dispatcher与repository同时关闭v1 production write，不双写v1/v2，不先开server后迁移。cutover后：只有名称明确的legacy reader/validator/migration API可读v1，绝不自动fallback；其它仍active的v1对象继续按自身lifecycle，不把“v1”整体legacy化。失败必须保持旧部署不可连接或回滚完整备份，不能进入半切换状态。
