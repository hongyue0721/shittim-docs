...
- `rustc 1.97.0`；
- `cargo 1.97.0`；
- Node **24.18.0** 的**实际入口**是 pnpm 用户 runtime：`~/.local/share/pnpm/bin/node`（将 `$HOME/.local/share/pnpm/bin` 放在 `PATH` 最前时，`node -v` 为 `v24.18.0`）；
...
3. Node 锁定 **24 LTS（exact 24.18.0）**。凡生成 lockfile、安装依赖或运行 Node 脚本，必须使用上述实际入口（`PATH` 优先 `~/.local/share/pnpm/bin`）；不得用默认 Node 26.x 生成产物后宣称满足约束。
...
- 根 Node/pnpm 基座已可验收：`export PATH="$HOME/.local/share/pnpm/bin:$PATH"` 后 `pnpm run check:toolchain` 应通过；用默认 Node 26 直接执行同一脚本或 `./scripts/check-schema.sh` 应在早期硬失败。
- 主仓→`shittim-docs` 纯文档镜像同步 library/CLI 已实现并完成首次真实发布验证：`scripts/sync-docs-repository.mjs`（`--check`/`--sync`/`--self-test`）+ `scripts/sync-docs-repository.test.mjs`（本地 bare remotes 的已列明覆盖）；npm scripts `check:docs-repository` / `sync:docs-repository` / `test:docs-repository`。`--check` 在 `/mnt/data` mode 0700 临时 bare 仓审计远端，不写 production docs checkout；source marker 以实时已推送 `master` first-parent 为权威；工具 fail-closed、普通 FF push、不 force、不提交主仓。首次发布由主仓 `dcf39df07d8deedcff5813368b92382bc499f98e` 同步到文档仓 `5dd8bf963e904ec8e54b3a41bef63868a724e932`，发布后检查为 `noop_idempotent`。仓库可见性 private 仍是人工事实；后续每个切片继续执行同一发布闭环。
- 仍无 TypeScript 业务包、无 TS Schema 生成、无 SDK/client、无 Tauri/React/AntD。
- Rust workspace 成员目前为 `kernel-contracts`、`schema-tool`、`domain-task`、`domain-policy`、`kernel-sqlite`、`kernel-kcp`、`kernel-task-creation` 与 `kernel-authorization`；仍无 `agentd`。
