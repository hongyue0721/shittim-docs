...
- [ADR-0007：Approval v2 不可变联合、身份与失效](0007-approval-v2不可变联合身份与失效.md) — accepted；**partial**（Approval/Identity repository已落地，真实远程验签与child materializer已落地；Provider 验签接入未做）；**部分被 ADR-0009 supersede**（v1 migration 生产要求）
...
- [ADR-0011：Action Lease 生命周期与 Stop Fence 原子语义](0011-lease生命周期与stop-fence原子语义.md) — accepted；**implemented**（migration 0010 持久化基座、`acquire_lease` / `get_action_lease` / `begin_dispatch` / `release_or_expire_lease` / Stop owner 与 Approval 撤销 Lease 全部落地；见 MATRIX/PROGRESS）
