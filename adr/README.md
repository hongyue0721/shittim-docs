...
- [ADR-0011：Action Lease 生命周期与 Stop Fence 原子语义](0011-lease生命周期与stop-fence原子语义.md) — accepted；**partial**（migration 0010 持久化基座与 `acquire_lease` / `get_action_lease` owner 已落地；`begin_dispatch` / `release_or_expire_lease` / Stop owner 未实施；见 MATRIX/PROGRESS）
