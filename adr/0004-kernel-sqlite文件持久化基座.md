...
6. Outbox 已由migration 0003升级为同表mixed v1/v2规范化存储：`payload_json`与`causation_json`均JCS，v1/v2共享aggregate sequence和AUTOINCREMENT position。`append_legacy_event_v1`与`append_active_event_v2`共用私有append内核；active type/aggregate由generated payload variant和`EVENT_ACTIVE_BINDINGS`派生。每次append在统一savepoint helper中分配sequence/position并最终严格readback；savepoint rollback/release cleanup失败会poison外层transaction，即使caller吞掉局部错误也不能commit。
7. Publisher存储面提供mixed按位置读取及幂等`mark_delivered`；第一次`delivered_at`不可覆盖。mark前按row version执行唯一严格stored decoder，损坏记录返回`stored_data_invalid`，不能以delivery mark隐藏。crate仍没有Publisher后台循环、删除、retention或claim lease。
...
- migration 0002实现legacy Task create/get；0003实现descriptor ledger升级与v1/v2统一Outbox；0004实现root TaskCreate v2 fresh-baseline表与active root repository（`create_root_task_v2`，消费`kernel-task-creation`并写`append_active_event_v2`）。legacy Task producer继续显式走v1 append，待后续切片删除。
