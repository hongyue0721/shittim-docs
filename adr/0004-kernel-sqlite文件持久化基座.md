...
3. migration 使用crate内嵌、严格升序descriptor。当前已实现0001/0002的legacy ledger为`schema_migrations(version,name,checksum,applied_at)`，checksum是SQL bytes SHA-256；ADR-0008规定0003在同一事务升级ledger并从0003起以稳定JCS descriptor单hash覆盖全部SQL assets与Rust transform identity/version，历史两行继续按旧算法验证。已应用名称/checksum/descriptor漂移返回`migration_drift`，数据库版本高于binary返回`database_schema_too_new`。每个pending migration在`BEGIN IMMEDIATE`中应用，业务DDL/transform与对应ledger行同事务提交或回滚；`schema_migrations` bootstrap在pending migration事务外独立幂等创建。
...
- 本 ADR 的后续migration 0002已实现Task create/get repository：Task/TaskScope/ContentOrigin canonical JSON单源、generated-column FK/index投影、关系数组镜像、task.create幂等与固定Audit/Event producer。该后续实现不改变本ADR对文件DB、事务、Audit与Outbox基座的历史决策。另见ADR-0008：active Event v2要求未来migration 0003把v1/v2放入同一Outbox并使用canonical `causation_json`；该决策不追溯改变本ADR已实现的v1代码事实，0003当前未实现。
