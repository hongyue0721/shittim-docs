...
| 5 | child Action materializer（migration 0011：原子物化 bundle + 五钉闭包 FK + 严格 readback；独立审查三轮闭合 0C/0H/0M） | **已完成** |
| 6 | §13.7 谓词闭合：`stop.activate` / `stop.status` handler（依赖 4–5）| **已完成**（`task.list` / `event.subscribe` / `event.poll` 仍缺 handler） |
...
- KCP 库级：`kernel-kcp` method-aware Value preflight、**五方法** registration/dispatcher/handler（`system.ping` / **active root `task.create` v2** / `task.get` / `stop.activate` / `stop.status`）与 SQLite adapter（`SqliteTaskBackend`→`create_root_task_v2`、`SqliteStopBackend`→`activate_stop_fence`/`get_stop_fence`）；不可连接，无 bytes/frame/server。
...
- **切片 5 child materializer 已落地**：`WriteTransaction::materialize_child_task`（migration 0011 原子物化 bundle + 五钉闭包 FK + strict readback + 幂等四分支 + approval resolution 消费全链），repository 闭集四方法（materialize / get_by_action / get_by_child_task / reconcile），独立审查三轮闭合 0C/0H/0M。
- Action 闭集其余写方法（child completion、recovery list；policy binding、lease acquire/dispatch/release 已实现）；真实远程验签 Provider 边界。
- 其它 active business **producer** 已全部有 owner（root/child `task.created`、`action.state_changed`、`approval.state_changed`、`stop_fence.activated`）。**Publisher** 与 versioned KCP **poll** 明确不在本里程碑。
- 其余 Command 幂等/乐观锁；**三方法**正式 handler（`task.list` / `event.subscribe` / `event.poll`）；Unix Domain Socket / Windows Named Pipe KCP **server/client**；**agentd**。
