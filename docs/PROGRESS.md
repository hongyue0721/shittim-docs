...
> 状态日期：2026-07-21（`V2InitialBuildActive`切片3c完成：删除 kernel-sqlite v1 runtime 写路径与 legacy Outbox append；Outbox v2-only；旧库 open `reinitialize-required`；migration 0005 drop 死 v1 表。§13.7 未闭合——child/Action/PD/Approval 与五方法 handler 仍缺。）
...
- 持久化：`kernel-sqlite` migration 0001–0005；AuditRecord **v2**；strict Task/TaskScope/ContentOrigin(v2) 读；版本化 **v2-only** Outbox + 严格 stored decoder + savepoint poison；**active root TaskCreate v2 repository**（`create_root_task_v2`）；legacy TaskCreate/Audit/Outbox v1 write 已删；旧库 open `reinitialize-required`。
...
- kcp runtime 与 sqlite repository 已切到 active create v2 / Outbox v2-only；五方法无 handler，禁止启动 server。§13.7 仍缺 child/Action/PD/Approval 闭环。
...
1. 切片 4–5：Action/PD/Approval/Identity repositories 与 child materializer；接入其余有 owner 的 active Event producers。
2. 切片 6：在 4–5 完成后闭合 §13.7 全部谓词。
3. **之后**再做 Publisher 与 versioned KCP poll；再实现剩余五个 Catalog handler 与可连接 server。
4. 随后实现 Extension SDK Base 与 TypeScript/client。

## 最近验证

本切片（V2InitialBuildActive 3c）验证命令：
