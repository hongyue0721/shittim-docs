...
| Event/SQLite Outbox | ADR-0008 accepted；v2八Schema、`change_kind` Approval真值表、Causation oneOf whole-ref、reserved claimant、`EventTypeBinding`单源bindings→types、exact 0003 descriptor与统一Outbox/KCP poll边界已闭合 | Event v2八Schema + retained v1 已进manifest/generated；catalog/typed decode已落地 | 当前v1 Outbox；migration 0003、fixed mixed API与active producer未实现 | Event v2 schema/claimant/catalog/typed tests已覆盖；migration/producer仍 future gate | 0003=`versioned_event_outbox`/single exact asset；单表、单position/sequence；retained poll v1不能返回v2 |
...
| 首批active事件 | 五类合同与ADR-0008权威闭合：新增`action.state_changed`与`approval.state_changed`，Approval有显式`change_kind`，Envelope/payload版本正交 | 八Schema + retained三payload + EventEnvelope v1/v2 已落地；`ActionTransitionIntentV1`仍后续独立批次 | v1 Outbox；无active producer/Publisher | Event claimant/payload/catalog/typed tests已覆盖；migration/producer未开始 | generated固定`EventTypeBinding`+active/legacy bindings并单源投影TYPE slices；`EVENT_V1_TYPES`已删除 |
| KCP 本地传输 | ADR accepted；受 typed-only 阶段门约束 | 不适用 | 未开始 | 未开始 | Unix Socket / Windows Named Pipe；本批不拍 path/frame 新事实、不允许可连接 server |
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、KCP V2 Envelope authority、production stage gate与task creation fixture合同已闭合；Event v2 claimant与binding Rust符号已实现 | production=61 entries（41 retained + 20 component-native）；production bindings `[]` | schema-tool已识别active Event claimant并生成`EventTypeBinding`/active-legacy bindings；pointer/CLI与Task helper已实现 | 生成链/fixture/Event v2 claimant/whole-schema mapping/bindings const projection/typed tests已覆盖 | 下一切片实现0003 Outbox与mixed API，不改本批Schema权威 |
