...
| AuditRecord | v1 legacy + active v2义务已定义 | AuditRecordV2与Schema-validated AuditAllocationV2已落地；v1 Schema/fixture retained | root task.create v2 写入 AuditRecordV2；v1 Store/write/producer 已删除（0005 drop 死表）；旧库 reinitialize-required | exact v2 wire + root v2 creation_recorded 投影一致性测试 | 其它 producer 的跨对象一致性仍未实现 |
| Event/SQLite Outbox | ADR-0008 accepted（§7/§8 legacy production API被ADR-0009 supersede）；v2八Schema与统一Outbox shape已闭合 | Event v2八Schema + retained v1 已进manifest/generated；catalog/typed decode已落地 | migration 0003–0005、descriptor v1、**v2-only** append/read、严格stored decoder、delivery gate与savepoint poison已实现；root task.create v2 是首个 active `task.created` producer；`append_legacy_event_v1`/`LegacyV1`已删；旧库 reinitialize-required；Publisher未实现 | descriptor/ledger/空表升级、reinitialize-required、v2-only事件、root v2 causation/correlation/readback、stored corruption与poison tests已覆盖 | retained poll v1不能返回v2；Publisher/poll不在V2InitialBuildActive |
...
| 首批active事件 | 五类合同与ADR-0008/0009权威闭合 | 八Schema + retained三payload + Envelope v1/v2 已落地 | v2-only Outbox append API已实现；root `task.created` 有 owner producer；child/action/approval producer与Publisher未实现 | v2-only五类active、aggregate mismatch及corrupt relation/reinitialize-required覆盖 | storage API不等于其余 producer；里程碑要求有owner的producer，不伪造 |
