...
| Schema生成链 | ADR accepted；manifest v2/walker/transaction完成；component-native exact、五值compatibility、MethodVersionBinding完整validator、V2 Envelope authority、production stage gate与task creation fixture合同已闭合 | production=53 entries（41 retained + 12 component-native）；`expires_at` pattern+format双门已生成；production bindings `[]`；三份official fixture wrapper已固化且不是business Schema | schema-tool生成Rust四生成物；中立strict RFC6901 selection/mutation、selected validate/canonicalize library与CLI `--pointer`、Bytes/Hex/Hash模式已实现；`kernel-task-creation` pure library与Task-specific harness已实现 | pointer/CLI、business-v2/graph、timestamp、inline normalization/hash/allocation，以及official root/child/allocation tamper与production owner + 独立CLI Schema路径 + stored preimage自一致性测试通过 | production bindings empty；repo/handler/materializer/cutover仍未实现 |
