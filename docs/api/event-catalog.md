...
> 状态徽章：**partial**（Event v2 八 Schema / catalog / typed decode 与 SQLite migration 0003 / 统一 Outbox shape **已实现**；V2InitialBuildActive切片1a又落地AuditRecordV2、AuditAllocationV2与TaskCreationProvenanceV1，明确Audit/Event正交；切片3c 起 production Outbox 为 **v2-only**（`append_legacy_event_v1` / `LegacyV1` 已删，旧库 reinitialize-required）；root `task.created` active producer 已接入；其余 active business producer、Publisher、versioned KCP poll **未实现**）
