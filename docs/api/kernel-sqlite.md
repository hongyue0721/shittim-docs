...
`rust/crates/kernel-sqlite`是文件型SQLite持久化crate，不是KCP或外部SDK API。当前实现migration 0001/0002/0003、AuditRecord v1、版本化统一Event Outbox、transaction-bound rate limit与**legacy TaskCreate v1** create/get repository。ADR-0006/0007要求的active业务repositories仍不存在；ADR-0008第二实现段只交付存储/migration/mixed API，不包含active business producer、Publisher或KCP poll cutover。
...
`WriteTransaction::create_task`仍是compatibility producer，现已显式调用`append_legacy_event_v1`，行为不变。它不代表active root v2 producer；active dispatcher/cutover不得复用legacy helper或增加可选v2分支。
...
active TaskCreate v2、Child Action materialization、Action/PermissionDecision/Approval repositories、active Event business producer、Publisher、KCP mixed poll、server/agentd、retention/claim lease仍未实现。Event v2 Schema与mixed storage API存在不等于这些业务能力可用。
